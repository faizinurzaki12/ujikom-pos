...
@push('styles')
    <link rel="stylesheet" href="{{ asset('assets/css/produk/index.css') }}">
@endpush

@section('content')
<div class="container-fluid px-0 produk-container">
        ...
        <table class="table table-hover align-middle mb-0">
            <thead class="table-dark">
                <tr>
                    ...
                    <th scope="col" class="text-center" style="width: 70px;">Foto</th>
            ...
            <tbody>
                ...
                <tr>
                    <th scope="row" data-label="#" class="text-center text-muted fw-normal">{{ $products->firstItem() + $loop->index }}</th>
                    <td data-label="User" class="fw-medium text-nowrap">{{ $product->user->name }}</td>
                    <td data-label="Jenis"><span class="badge bg-light text-dark border">{{ $product->jenis->nama_jenis ?? '-' }}</span></td>
                    <td data-label="Foto" class="text-center">
                        @if($product->foto)
                            <img src="{{ asset('storage/' . $product->foto) }}" width="40" height="40" alt="{{ $product->nama }}" class="img-thumbnail object-fit-cover rounded-2">
                        @else
                            <span class="badge bg-secondary-subtle text-secondary fs-7"><i class="bi bi-image"></i></span>
                        @endif
                    </td>
                    <td data-label="Nama" class="fw-semibold text-nowrap">{{ $product->nama }}</td>
                    <td data-label="Harga Beli" class="text-end text-nowrap">Rp {{ number_format($product->harga_beli, 0, ',', '.') }}</td>
                    <td data-label="Harga Jual" class="text-end text-nowrap fw-semibold text-primary">Rp {{ number_format($product->harga_jual, 0, ',', '.') }}</td>
