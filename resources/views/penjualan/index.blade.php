...
@push('styles')
    <link rel="stylesheet" href="{{ asset('assets/css/penjualan/index.css') }}">
@endpush
    ...
    <div class="table-responsive rounded-3 border shadow-sm mb-3">
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                ...
                <tr>
                    <th scope="row" data-label="#" class="text-center text-muted fw-normal">{{ $sales->firstItem() + $loop->index }}</th>
                    <td data-label="Tanggal" class="text-nowrap">{{ $sale->created_at->translatedFormat('d-m-Y H:i') }}</td>
                    <td data-label="Kasir" class="fw-medium">{{ $sale->user->name }}</td>
                    <td data-label="Produk">
                        <ul class="list-unstyled mb-0 small">
                            @foreach($sale->itemPenjualan as $item)
                                <li>{{ $item->produk->nama ?? 'produk Dihapus '}}</li>
                            @endforeach
                        </ul>
                    </td>
                    <td data-label="Total Pembayaran" class="text-end text-nowrap fw-bold text-success">
